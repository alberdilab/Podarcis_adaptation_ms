...
richness_accli %>%
  ...
  scale_shape_manual(name="Type",
                     breaks=c("Cold_ontrol", "Warm_control", "Cold_intervention"),
                     labels=c("Cold_ontrol", "Warm_control", "Cold_intervention"),
...
```{r struct_zero_ci_accli_fmt1_1, comment="", echo=FALSE, message=FALSE, warning=FALSE}
accli_ci_samples <- sample_metadata %>% 
  filter(Population == "Cold") %>% 
  filter(time_point == "Acclimation") %>%
  dplyr::select("Tube_code") %>%
  pull()

fmt1_ci_samples <- sample_metadata %>% 
  filter(Population == "Warm") %>% 
  filter(time_point == "Acclimation")%>%
  dplyr::select("Tube_code") %>%
  pull()

meta_accli_ <- sample_metadata%>% 
  filter(Population %in% c("Cold", "Warm") & time_point == "Acclimation")
...
structural_zeros <- genome_counts_filt %>%
  ...
  mutate(present = case_when(all_zeros_accli_ci & !all_zeros_ci_fmt1 ~ "Warm",!all_zeros_accli_ci & all_zeros_ci_fmt1 ~ "Cold",!all_zeros_accli_ci & !all_zeros_ci_fmt1 ~ "None",TRUE ~ NA_character_)) %>%
  mutate(average = ifelse(present == "Cold", average_accli_ci, average_ci_fmt1)) %>%
...
```{r structural_zeros_acci_fmt1_ci_2}
struc <- structural_zeros %>% 
  filter(present=="Cold")%>% 
  count(phylum, name = "Cold") %>%
  arrange(desc(Cold))
structural_zeros %>% 
  filter(present=="Warm")%>% 
  count(phylum, name = "Warm") %>%
  arrange(desc(Warm)) %>% 
...
```{r structural_zeros_acci_fmt1_ci_3}
...
```{r ancom_rand_pond_accli_fmt1_ci_1, comment="", message=FALSE, warning=FALSE, eval=FALSE}
...
```{r ancom_rand_FMT1_accli_ci_1, comment="", echo=TRUE, message=FALSE, warning=FALSE}
...
```{r ancombc_rand_plot_phy_FMT1_accli_ci_1, comment="", echo=TRUE, message=FALSE, warning=FALSE, fig.height=5, fig.width=6, fig.fullwidth=TRUE}
ancombc_rand_table_mag %>%
  mutate(
