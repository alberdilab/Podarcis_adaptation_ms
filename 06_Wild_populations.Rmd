...
structural_zeros <- genome_counts_filt %>%
  ...
  mutate(present = case_when(all_zeros_cold & !all_zeros_warm ~ "Warm",!all_zeros_cold & all_zeros_warm ~ "Cold",!all_zeros_cold & !all_zeros_warm ~ "None",TRUE ~ NA_character_)) %>%
...
ancom_rand_output_wild_phylum$res %>%
  dplyr::select(taxon, lfc_PopulationWarm, p_PopulationWarm) %>%
  filter(p_PopulationWarm < 0.05)
```
...
```{r ancom_rand_1, comment="", echo=FALSE, message=FALSE, warning=FALSE}
