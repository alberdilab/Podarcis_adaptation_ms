...
table_upset_analysis_cont=t(aggregate(t(genome_counts_rel_pa),by=list(subset_meta$species.y),FUN=sum)[,-1])
colnames(table_upset_analysis_cont)=levels(as.factor(subset_meta$species.y))
...
upset(as.data.frame(table_upset_analysis),
  keep.order = T,
  sets = rev(c("Podarcis_muralis","Podarcis_liolepis")),
  sets.bar.color= rev(speciescolors),
...
significant_elements_wild <- element_gift_wild %>%
  pivot_longer(-c(Tube_code, species),
               names_to = "trait",
               values_to = "value") %>%
  group_by(trait) %>%
...
difference_table_wild <- element_gift_sig_wild %>%
  ...
  arrange(Function) %>%
  mutate(Difference = Podarcis_muralis - Podarcis_liolepis) %>%
  mutate(group_color = ifelse(Difference < 0, "Podarcis_liolepis", "Podarcis_muralis")) 
```
