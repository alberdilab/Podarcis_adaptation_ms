...
alpha_div%>%
  ...
  scale_color_manual(values=c("#d57d2c",'#008080')) +
  scale_fill_manual(values=c("#d57d2c50",'#00808050')) +
...
#Richness
cca_ord <- capscale(formula = richness ~ subset_meta$time_point* subset_meta$species)
...
biplot_scores$Variable <- recode(
  ...
  "subset_meta$speciesPodarcis_liolepis" = "Podarcis_liolepis",
  "subset_meta$time_pointAntibiotics:subset_meta$speciesPodarcis_liolepis" =
    "Interaction Podarcis_liolepis"
)
...
CAP_df %>%
  ...
  mutate(y_cen = mean(CAP2, na.rm = TRUE)) %>%
  ungroup() %>%
  ggplot(., aes(x=CAP1,y=CAP2, color=species,shape = time_point)) +
...
#Neutral
cca_ord <- capscale(formula = neutral ~ subset_meta$time_point* subset_meta$species)
CAP_df <- as.data.frame(vegan::scores(cca_ord, display = "sites")) %>%
  rownames_to_column('Tube_code') %>%
  left_join(subset_meta, by = 'Tube_code') %>%
  column_to_rownames('Tube_code')%>%
...
# Does antibiotic administration remove the differences found in the two species?
